...
def builddictionary():
    wb = load_workbook(filename='dictionary_csv/dictionary.xlsx', read_only=True)
    ws = wb['Sheet1']
    translatelist1 = {}
    for row in ws.rows:
...
def write2docx(Ingredients_length_in_one_row,Ingredients_length_in_one_col,Chinese_Name,Ingredients_name_list,product_weight_number,product_weight,product_time,expire_time,Chinese_ProductPlace):
    ...
    translatelist1 = builddictionary()
...
for col in ws.rows:
    ...
    if col[0].value==None:
        ...
        Ingredients_name_list = get_Ingredients_name_list(col[1].value)
        ...
        print(product_weight_number)
        print(product_weight)
        print(product_time)
        print(expire_time)
        print(Chinese_ProductPlace)
        rownumber=col[6].value
